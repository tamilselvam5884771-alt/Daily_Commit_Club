...
/**
 * Ornate Member Information Card
 * Fantasy Storybook styled modal card displaying member profile, streak, today's commit breakdown, repos, and coffee debt.
 */
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Ornate Header Header & Character */}
        <div className="flex flex-col items-center text-center">
          <Character user={user} size="lg" speechText={`Welcome to ${building.name || 'my realm'}!`} />
        ...
        <div className="mt-3 p-3 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-between text-xs">
          <div>
            ...
            <span className={`font-bold ${building.health > 60 ? 'text-emerald-400' : 'text-rose-400'}`}>
              {building.health ?? 100}% {building.destroyed ? '(DESTROYED)' : ''}
            </span>
          </div>

          <div className="flex items-center gap-1 text-amber-300 font-semibold">
            <Coffee className="w-4 h-4 text-amber-400" /> Coffee Owed: {user.coffeeDebt || 0}
          </div>
        </div>
